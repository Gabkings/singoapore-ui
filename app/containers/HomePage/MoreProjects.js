...
class MostPopularProjects extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <Section style={{ backgroundColor: '#FAFAFA', marginTop: '30px' }}>
        <div id="egg-shape">{this.renderShape()}</div>
        <Subsection style={{ width: '77%', marginTop: '-30px' }}>
          <div style={{ margin: '30px 30px 20px', textAlign: 'left' }}>
            <h2>More Services</h2>
          </div>
          ...
          <Grid
            columns={3}
            doubling
            style={{ margin: '0px 30px 5px' }}
          >
            <Grid.Row className="grid-row-more-project" style={{ paddingTop: 0, paddingBottom: 0 }}>
              {popularList.map(p => (
                <Grid.Column key={v4()} id="grid-col-more-project">
                  <LinkWrapper
                  ...
                  >
                    <div className="wrap">
                      <img
                        ...
                        className='wrap-img'
                      />
                      <p className='wrap-p'>{categoriesMap[p].name}</p>
                    </div>
                  </LinkWrapper>
                </Grid.Column>
              ))}
          ...
          <LinkWrapper href="/services">
            <ButtonWrapper style={{ margin: '30px 0px' }} design="filled">View All</ButtonWrapper>
          </LinkWrapper>
        </Subsection>
      </Section>
    )
  ...
  componentWillReceiveProps(nextProps) {
    ...
    popularList.sort((a, b) => categoriesMap[a].popular - categoriesMap[b].popular);
    let listLength = popularList.length
    while (listLength > 0) {
      if(listLength <= 12) {
        popularList.shift()
      }
